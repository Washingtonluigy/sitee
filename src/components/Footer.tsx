      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-12 md:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12 mb-8 md:mb-12">
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xl md:text-2xl font-bold text-white" translate="no">Amah</span>
            </div>
            <p className="text-xs md:text-sm text-gray-400 leading-relaxed mb-4 md:mb-6" translate="no">
          ...
          <div>
            <h3 className="text-white font-bold text-base md:text-lg mb-3 md:mb-4" translate="no">Plataforma</h3>
            <ul className="space-y-2 md:space-y-3 text-xs md:text-sm">
              {getLinksBySection('Plataforma').map(link => (
                <li key={link.id}>
                  <a href="#" onClick={(e) => { e.preventDefault(); window.location.href = link.url; }} className="hover:text-purple-400 transition-colors" translate="no">
                    {link.title}
                  </a>
                </li>
              ))}
          ...
          <div>
            <h3 className="text-white font-bold text-base md:text-lg mb-3 md:mb-4" translate="no">Empresa</h3>
            <ul className="space-y-2 md:space-y-3 text-xs md:text-sm">
          ...
          <div>
            <h3 className="text-white font-bold text-base md:text-lg mb-3 md:mb-4" translate="no">Contato</h3>
            <ul className="space-y-3 md:space-y-4 text-xs md:text-sm">
              {contactInfo?.email && (
                <li className="flex items-start gap-2 md:gap-3">
                  <Mail className="w-4 md:w-5 h-4 md:h-5 text-purple-400 flex-shrink-0 mt-0.5" />
                  <span translate="no">{contactInfo.email}</span>
                </li>
              )}
              {contactInfo?.phone && (
                <li className="flex items-start gap-2 md:gap-3">
                  <Phone className="w-4 md:w-5 h-4 md:h-5 text-purple-400 flex-shrink-0 mt-0.5" />
                  <span translate="no">{contactInfo.phone}</span>
                </li>
              )}
        ...
        <div className="border-t border-gray-700 pt-6 md:pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-3 md:gap-4 text-center md:text-left">
            <p className="text-gray-400 text-xs md:text-sm" translate="no">
            ...
            <div className="flex gap-4 md:gap-6 text-xs md:text-sm flex-wrap justify-center">
              {contactInfo?.terms_url && (
                <a href="#" onClick={(e) => { e.preventDefault(); window.location.href = contactInfo.terms_url; }} className="hover:text-purple-400 transition-colors" translate="no">Termos de Uso</a>
              )}
              {contactInfo?.privacy_url && (
                <a href="#" onClick={(e) => { e.preventDefault(); window.location.href = contactInfo.privacy_url; }} className="hover:text-purple-400 transition-colors" translate="no">Política de Privacidade</a>
              )}
              {contactInfo?.cookies_url && (
                <a href="#" onClick={(e) => { e.preventDefault(); window.location.href = contactInfo.cookies_url; }} className="hover:text-purple-400 transition-colors" translate="no">Cookies</a>
              )}
            </div>
          </div>
        </div>
